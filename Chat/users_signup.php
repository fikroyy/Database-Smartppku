...
$position = array("Sebagai","Mahasiswa","Mentor");

$deparment = array("Jurusan","FAPERTA","FKH","FPIK","FAPET","FAHUTAN","FATETA","FMIPA","FEM","FEMA","SB");
  ...
  if(isset($_POST["submit"])){
    ...
    if($first_name!="" && $last_name!="" && $user_password!="" && $user_position!="" && $user_department!=""){

      $security_key_check_command = "SELECT * FROM users_security_keys WHERE users_fname='$first_name' AND users_lname = '$last_name'";
      ...
      if($check_validity_status["status"] == "not_used"){
        ...
        echo "<script>alert('You have registered Successfully')</script><script>document.location.href='login.php';</script>";
      }else{
...
<body>
  ...
  <h3 style="font-size:1em;color:#2dc6bf;text-align:center;cursor:hand"><u><a href="index.html">SMART PPKU</a></u>
  ...
  <input class='lf--submit lf--submit-adjusted' type='submit' value='Daftar' name="submit" id="btnSubmit">

  <p style="font-size:.6em;color:#2dc6bf;text-align:center;cursor:hand"><u><a href="login.php">Masuk</a></u></p>
